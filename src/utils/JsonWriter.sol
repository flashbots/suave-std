...
// Copied from: https://github.com/bmeredith/solidity-json-writer
...
library JsonWriter {
    ...
    /**
     * @dev Writes the property name and bytes value (as a JSON string) as part of a name/value pair of a JSON object.
     */
    function writeBytesProperty(Json memory json, string memory propertyName, bytes memory value)
        internal
        pure
        returns (Json memory)
    {
        if (json.depthBitTracker < 0) {
            json.value = string(
                abi.encodePacked(json.value, LIST_SEPARATOR, '"', propertyName, '": "', Base64.encode(value), '"')
            );
        } else {
            json.value = string(abi.encodePacked(json.value, '"', propertyName, '": "', Base64.encode(value), '"'));
        }
    ...
    /**
     * @dev Writes the bytes value (as a JSON string) as an element of a JSON array.
     */
    function writeBytesValue(Json memory json, bytes memory value) internal pure returns (Json memory) {
        if (json.depthBitTracker < 0) {
            json.value = string(abi.encodePacked(json.value, LIST_SEPARATOR, '"', Base64.encode(value), '"'));
        } else {
            json.value = string(abi.encodePacked(json.value, '"', Base64.encode(value), '"'));
        }
     ...
     */
    function writeStart(Json memory json, string memory propertyName, bytes1 token)
        private
        pure
        returns (Json memory)
    {
