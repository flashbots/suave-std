...
library Bundle {
    ...
    function encodeBundle(BundleObj memory args) internal pure returns (Suave.HttpRequest memory) {
        ...
        string memory params =
            string.concat('{"blockNumber": "', LibString.toHexString(args.blockNumber), '", "txs": [');
        for (uint256 i = 0; i < args.txns.length; i++) {
            params = string.concat(params, '"', LibString.toHexString(args.txns[i]), '"');
        ...
        if (args.minTimestamp > 0) {
            params = string.concat(params, ', "minTimestamp": ', LibString.toString(args.minTimestamp));
        }
        if (args.maxTimestamp > 0) {
            params = string.concat(params, ', "maxTimestamp": ', LibString.toString(args.maxTimestamp));
        }
        params = string.concat(params, "}");

        string memory body = string.concat('{"jsonrpc":"2.0","method":"eth_sendBundle","params":[', params, '],"id":1}');
